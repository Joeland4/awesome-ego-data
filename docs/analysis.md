...
**已解析小时数的数据集合计 ≈ 134,853 小时（≈15.3 年连续视频）。** 这是**下界**，多数新数据集（Gen-HumanEgo 1,800h 未计入、WIYH 以“条”计、Demo 类无时长）未纳入。

### ⚠️ 按访问类型拆分（关键：宣称 ≠ 可获取）

| 访问类型 | 小时 | 数据集数 | 说明 |
|---|---|---|---|
| **open（HF 公开可直接下载）** | **9,215 h** | 55 | 真正能立刻拿到的 |
| gated（HF 需申请） | 20,682 h | 17 | 登录+同意条款 |
| request（官网/DUA） | 104,956 h | 3 | 光轮 EgoSuite 一家 100,000h |
| **合计** | **134,853 h** | 74 | — |

> **真正 open 的仅 ~8,700 h，占总量的 6.5%。** 13.4 万 h 的“总量”主要来自**单一 request-only 宣称值**（EgoSuite-Open100K 的采集总量，首批并未全量上线）。任何“Ego 数据已超 13 万小时”的说法都应附带访问类型。
